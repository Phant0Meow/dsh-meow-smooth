...
- **返回手势当抽屉用**：手机浏览器里，屏幕左缘往右一划（或按返回键）不再退出页面——直接打开侧边栏；边栏开着时再来一次就收起。跟 PWA 里的体验一致，AI 跑着任务也绝不会被一滑带走；要离开页面用系统的多任务/标签切换。
...
- **宽手机右侧面板分屏（dsh 0.1.6+）**：折叠屏内屏、小平板（宽度约 560–768px）上打开右上角的文件查看面板，不再整屏盖住对话——屏幕左右各半：左边继续对话，右边看文件，面板滑入与对话让位是同步动画。普通手机（≤480px）保持全屏；电脑窄窗口与 ≥768px 设备官方本来就是分栏，行为不变。
- **细竖条折成小方块**：手机上连收起后的那条细竖条也省了——它折叠成左上角一个带 DeepSeek 图标的小方块，会话标题自动让位，谁也不挡谁。点一下直接展开完整侧边栏，不再先唤出细竖条，选完会话自动折回去。细竖条仍可从屏幕左缘划出来停留。
