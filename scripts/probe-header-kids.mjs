/** 探针：header 子元素结构——找到画淡灰线的元素（标签行轨道下边线）。 */
...
const PORT = 9363
const profile = join(process.env.TEMP ?? '.', `meow-probe8-${Date.now()}`)
  ...
  const dump = await evalJson(`(function(){
    ...
    const describe = (el) => {
      ...
      return {
        ...
        top: +(r.top - hRect.top).toFixed(1),
        bottom: +(r.bottom - hRect.top).toFixed(1),
        h: +r.height.toFixed(1),
        bb: cs.borderBottomWidth + ' ' + cs.borderBottomColor,
        padB: cs.paddingBottom,
      }
    }
    const kids = [...header.children].map(describe)
    const last = header.lastElementChild
    const lastKids = last !== null ? [...last.children].map(describe) : []
    return JSON.stringify({
      header: { h: +hRect.height.toFixed(1), padB: getComputedStyle(header).paddingBottom, bb: getComputedStyle(header).borderBottomWidth },
      kids,
      lastChildKids: lastKids,
      lastBottom: last !== null ? +(last.getBoundingClientRect().bottom - hRect.top).toFixed(1) : null,
    })
  })()`)
